...
export function WarLayout({ header, feed, desk }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[40%_60%]">
        {/* Mobile / tablet: desk first so Intel + ?t= is visible without scrolling past the whole feed */}
        <div className="order-2 min-h-0 overflow-y-auto overflow-x-hidden border-slate-800/80 lg:order-1 lg:border-r">
          {feed}
        </div>
        <div className="order-1 min-h-0 max-h-[42dvh] overflow-y-auto overflow-x-hidden border-b border-slate-800/80 lg:order-2 lg:max-h-none lg:border-b-0 lg:border-t-0">
          {desk}
        </div>
      </div>
    </div>
