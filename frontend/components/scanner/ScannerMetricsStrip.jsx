...
export function ScannerMetricsStrip({ token, universeRows, t }) {
  ...
  return (
    <div className="grid gap-2 border-b border-white/10 px-4 py-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
      <MetricCell tone="liquidity" label={t("scanner.metric.liquidity")} value={`$${liq.toLocaleString()}`} />
      <MetricCell tone="volume" label={t("scanner.metric.volume")} value={`$${vol.toLocaleString()}`} />
